...
###   Define functions for simulation runs                        ###
...
# initMean = trait mean value at t = 0 

# Calculate input value table for model
gSetup<- function(f, h2, Ve, nP, nN, nI, nH){
  Vg<- h2*Ve/(1-h2)
  eSize<- sqrt(Vg/(2*nP*f*(1-f)))
  initMean<- 2*f*nP*eSize
  lLoci<- runif(sum(nP, nN, nI, nH), 0, 1)
  initSD<- sqrt(Vg+Ve)
  list(eSize=eSize, Vg=Vg, Ve=Ve, initMean=initMean, lLoci=lLoci, initSD=initSD)
}

# Initialise the simulation with a array of individuals of dimensions [n, (nP+nI+nH+nN), 2]
...
initInds<-function(n , nP, nN, nI, nH, f, recipient=TRUE){
  ...
  # Define phenotypic allele values
  allelesP1 <- matrix(rbinom(n*nP, 1, f), nrow=n, dimnames=list(NULL, rep("P", nP)))
  allelesP2 <- matrix(rbinom(n*nP, 1, f), nrow=n, dimnames=list(NULL, rep("P", nP)))
  
  # Define heterozygosity allele values
  if (recipient) gCode <- 1 else gCode<- 20
  allelesH1 <- matrix(rbinom(n*nH, gCode, 0.5), nrow=n, dimnames=list(NULL, rep("H", nH)))
  allelesH2 <- matrix(rbinom(n*nH, gCode, 0.5), nrow=n, dimnames=list(NULL, rep("H", nH)))
  
  # Define incompatibility and neutral allele values
  if (recipient) gCode<- 0 else gCode<- 1
  allelesI1 <- matrix(rep(gCode, n*nI), nrow=n, dimnames=list(NULL, rep("I", nI)))
  allelesI2 <- matrix(rep(gCode, n*nI), nrow=n, dimnames=list(NULL, rep("I", nI)))
  allelesN1 <- matrix(rep(gCode, n*nN), nrow=n, dimnames=list(NULL, rep("N", nN)))
  allelesN2 <- matrix(rep(gCode, n*nN), nrow=n, dimnames=list(NULL, rep("N", nN)))
  
  # Bind simulated chromosomes
...
# Calculate the phenotype for each row in the array, based on sum of nP alleles and effect size
...
# m dictates the width [slope] of the response i.e. a flattening term
...
# Defines a linear environmental shift in the model
optShiftLine<- function(gens, initSD){
  x<-seq(from = 0, to = initSD*3.722, by = initSD*3.722/gens) #3.722 to equal max rate of change for m=0.15 
  x
}
...
# phen is drawn from each generations mean phenotype
...
gTracker<- function(pop, gVars, nLoci){
  ...
  gCols<- grepl("H", colnames(pop))
  nCols<- sum(gCols)
  gCols<- pop[, gCols, , drop = FALSE] #genotypes
  n1<-apply(gCols==1, 2, sum)
  n0<-apply(gCols==0, 2, sum)
  homoP<-(n1/(n1+n0))*((n1-1)/(n1+(n0-1)))+(n0/(n1+n0))*((n0-1)/(n1+(n0-1)))
  SDI<- 1-mean(homoP)
  ...
  nLoci<- nCols
  gCols<- pop[, gCols, , drop = FALSE] #genotypes
  femAllele<- gCols[, , 1, drop = FALSE] #relevant subset
  maleAllele<- gCols[, , 2, drop = FALSE]
  homozygous<- femAllele==maleAllele #conditions
  homP1<- homozygous & femAllele == 0
  homP2<- homozygous & femAllele == 1
...
evolve<- function(initN, nP, nN, nI, nH, initFreq, h2, Ve, gens, Rmax, Nstar, decay, recombRate, m, nIntro, TGFtime, nLoci, H2, outbreeding, alpha, burnIn = 10, type){
  
  #initilise the population
  pop<- initInds(initN, nP, nN, nI, nH, f=initFreq)
  
  #global parameters
  gVars<- gSetup(initFreq, h2, Ve, nP, nN, nI, nH)
  if (change == 1){
  opt<<- optShiftLine(initSD=gVars$initSD, gens)
  } else {
